		...
		?>
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					...
					if (count($custom_fields)) {
						$fields_ids = explode(',', $row['conf_scr_custom_ids']);
						$fields_only_edition_ids = explode(',', $row['cfields_only_edition']);
						$fields_user_hidden_ids = explode(',', $row['cfields_user_hidden']);
					?>
						<div class="w-100">
							<p class="h6 text-center font-weight-bold mt-4"><?= TRANS('EXTRA_FIELDS'); ?></p>
						</div>
