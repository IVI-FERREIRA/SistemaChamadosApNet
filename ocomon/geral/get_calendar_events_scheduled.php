...
$sql = "SELECT 
    ...
    o.oco_scheduled_to >= :start AND 
    o.oco_scheduled_to <= :end 
    {$terms}
    {$terms2}
    {$filter_areas}
    ";
...
foreach ($res->fetchAll() AS $row) {
    ...
    // $data[$i]['descricao'] = html2text($row['descricao']);
    ...
    $data[$i]['oco_scheduled_to'] = dateScreen($row['oco_scheduled_to']);
    $data[$i]['data_fechamento'] = '';
