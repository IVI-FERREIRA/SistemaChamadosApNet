...
if (!empty($aux_worker)) {
	foreach ($aux_worker as $worker) {
		if (strlen($aux_workers)) $aux_workers .= ', ';
		$aux_workers .= $worker['nome'];
	}
}

$customFields = [];
$customFields = getTicketCustomFields($conn, $_GET['numero']);
...
if ((isset($_SESSION['s_nivel']) && $_SESSION['s_nivel'] == 3) || $GLOBALACCESS) {
	...
	$hiddenCustomFields = ($profile_id ? explode(',',getScreenInfo($conn, $profile_id)['cfields_user_hidden']) : []);
}
