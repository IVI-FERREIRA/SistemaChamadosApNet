...
$sql = "SELECT 
    ...
    o.data_fechamento >= :start AND 
    o.data_fechamento <= :end 
    {$terms}
    {$terms2}
    {$filter_areas}
    ";
...
foreach ($res->fetchAll() AS $row) {
    ...
    $data[$i]['start'] = $row['data_fechamento'];
    ...
    $data[$i]['oco_scheduled_to'] = '';
    $data[$i]['data_fechamento'] = dateScreen($row['data_fechamento']);
