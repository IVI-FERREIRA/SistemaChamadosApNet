    ...
    <div class="container-fluid">
        ...
        if (($_SESSION['s_nivel'] != 1) && !empty($_SESSION['s_allowed_units'])) {
            ...
            foreach ($arrayFilteredUnitsNames as $filteredUnit) {
                if (strlen($filteredUnitsNames) > 0) {
                    $filteredUnitsNames .= ", ";
                } else {
        ...
        foreach ($resultado->fetchall() as $row) {
            ...
            <tr class=" table-borderless">
                ...
                <td class="line"><span class="asset-info" data-qstring="equip_type=<?= $row['tipo']; ?>&model=<?= $row['modelo_cod']; ?>"><?= $row['Equipamento'];?></span></td>
                <td class="line"><span class="asset-info" data-qstring="equip_type=<?= $row['tipo']; ?>&model=<?= $row['modelo_cod']; ?>"><?= $row['fabricante'] . " " . $row['modelo']?></span></td>
