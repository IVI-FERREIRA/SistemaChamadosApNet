...
if (empty($criteria['units'])) {
    ...
    foreach ($criteria['units'] as $unit) {
        if (strlen($units_names) > 0) $units_names .= ', ';
        $units_names .= $unit;
    }
...
if (empty($criteria['categories'])) {
    ...
    foreach ($criteria['categories'] as $categorie) {
        if (strlen($categories_names) > 0) $categories_names .= ', ';
...
if (empty($criteria['asset_type'])) {
    ...
    foreach ($criteria['asset_type'] as $type) {
        if (strlen($types_names) > 0) $types_names .= ', ';
        $types_names .= $type;
    }
...
$sql = "SELECT 
            ...
            COALESCE (r.reit_nome, 'N/A') AS rectory
        FROM 
            equipamentos e, 
            tipo_equip t, 
            instituicao u, 
            localizacao d
                LEFT JOIN reitorias r ON r.reit_cod = d.loc_reitoria
        WHERE
